...
- [β E2E サインオフ](docs/beta-e2e-signoff.md) — 拡張保存→Entry管理フローの動作確認手順

## 開発ステータス（β）

βの完成条件「Chrome 拡張で保存した求人ページを、ログインユーザーが Web の Inbox から Entry 化して管理できる」を満たした状態。

実装済み:

- Google ログイン（Firebase セッション Cookie）
- Chrome 拡張から Inbox API への保存（未ログイン・保存失敗時の回復導線つき）
- Inbox clip の一覧 / Entry 化 / 削除、重複 URL の最小対応
- Entry 一覧 / Kanban / 詳細（会社名表示）
- Task 画面（実 API データの表示・状態更新）
- Dashboard / Sidebar の実データ表示
- ローカル MCP server（Entry / Inbox / Task 参照、Task作成、ESメモ蓄積、メール本文のルールベース抽出）
