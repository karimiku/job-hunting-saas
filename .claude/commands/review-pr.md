...
### PR 体裁観点（必須チェック）
- **[pr-format]**: PR 本文が `.github/pull_request_template.md` の見出し構成（概要 / 変更内容 / 設計判断・意思決定の理由 / 関連issue / レビュー観点 / フロー図(任意) / テスト）に従っているか。PR タイトルが日本語か（prefix `feat:` 等は可）。違反していれば【必須】で指摘する。
