...
- `main` branchから実行する
- `confirm_production_deploy` に `deploy-production` と入力する
...
2. `Run workflow` を選ぶ
3. Branchは `main` を選ぶ
4. `confirm_production_deploy` に `deploy-production` と入力する
5. `image_tag` は通常空欄のままにする
6. Artifact Registry pushとCloud Run deployが実行される
7. workflow内の `/health` smoke testが成功することを確認する
...
```text
RATE_LIMIT_GLOBAL_REQUESTS_PER_MINUTE=120
RATE_LIMIT_AUTH_REQUESTS_PER_MINUTE=10
RATE_LIMIT_AUTHENTICATED_REQUESTS_PER_MINUTE=300
```
