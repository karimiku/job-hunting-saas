...
2026-06-15 に通常APIレスポンスへ `Server-Timing` を追加した。

主に見る指標:

```text
firebase_verify_session_cookie  Firebase session cookie検証
external_identity_lookup        Firebase UID -> user_id 解決
auth_bearer                     Bearer token認証
app                             Go APIのアプリ処理全体
```

Next Server側では `ENTRE_SERVER_FETCH_LOG=1` を設定すると、Server Component / Server Action からの backend fetch がログに出る。

```text
[serverFetch] GET /api/v1/tasks -> 200 123ms server-timing="firebase_verify_session_cookie;dur=..., external_identity_lookup;dur=..., app;dur=..."
```

このログで、RSCの遅さを以下に分解する:

```text
serverFetch duration
  - Server-Timing app
    - firebase_verify_session_cookie
    - external_identity_lookup
    - handler/usecase/sql
  - Vercel Next Server -> Cloud Run のネットワーク往復
  - response body parse
```

判定の目安:

- `firebase_verify_session_cookie` が大きい: 通常APIごとのFirebase失効チェックが固定費になっている可能性が高い
- `external_identity_lookup` が大きい: 認証ごとのDB lookup、DB接続待ち、DBリージョン距離を疑う
- `app` は小さいが `serverFetch duration` が大きい: Vercel -> Cloud Run のネットワーク、Cloud Run cold start、TLS/接続確立を疑う
- `app` が大きく、認証指標は小さい: handler/usecase/sql またはJSON encodeを疑う
