...
2. **すべてのコア機能は Web の手動入力で完結できる**ことを前提とし、Chrome拡張は任意の入力補助として扱う
3. Task（作業・予定）で締切と日程を一元管理し、漏れを実質ゼロに近づける
4. ホーム / Entry / カンバン / タスク / 保存箱に導線を絞り、次にやることを迷わず判断できる
...
- Clip（証跡URL・タイトル等）と保存箱（Entry候補の一時置き場）
- 保存箱 → Entry 化 → カンバン/タスク管理
- ホーム（次の行動 + 今日のタスク）
- レスポンシブ（スマホ閲覧 + Entry追加 + タスク処理）

### 6.2 βの任意入力補助

- Chrome拡張: URL・タイトル自動取得 + source推定 + 企業名候補提示（保存箱への入力補助）
- 重複統合（別名辞書の活用）
- 拡張の「直近Entryに即保存」導線
...
### 代表的シーン
1. 保存箱に求人を残し、必要なものだけ Entry に変換する
2. Entry一覧とカンバンで、応募先と選考フェーズを俯瞰する
3. スマホで今日のタスクを確認し、完了/期限変更/メモ追記を行う
4. 応募詳細から証跡を辿ってマイページへ戻り、迷わず次作業に進む
...
- **入力最小化**: 保存箱を挟み、必要な求人だけを Entry 化することで入力判断を遅らせられる
...
| FR-04a | アカウント登録フローでは Web のコア導線を優先し、Chrome拡張は任意の入力補助として案内する |
...
| FR-42 | 期限/日時（due_date、任意） |
| FR-43 | ホームで未完了Taskを確認できる |
| FR-44 | Entry詳細とTask画面の両方からTaskを処理できる |

### 9.6 Clip（証跡）・Inbox（未整理箱）
...
| ID | 要件 |
|----|------|
| FR-90 | レスポンシブでホーム / Entry / カンバン / Task / 保存箱を閲覧 |
...
User
 ...
 │    ├── 1:N Task（作業・予定を統合）
 │    ├── 1:N Clip（entry_id ありの証跡）
 │    └── 1:N StageHistory
 └── 1:N Clip（entry_id なし = Inbox）
```
...
| Clip | id, user_id, entry_id(nullable), url, page_title, source, memo, archived, created_at | 実装済み |
...
- 次の行動を1つだけ提示
- 今日のタスクを表示
- 保存箱 / Entry / Task への導線を状況に応じて出し分け
- 未整理Clip件数をナビ/CTAで確認
...
### カンバン
- Entryを選考ステージ別の列で表示
- Entryをドラッグ&ドロップでステージ変更
- 会社名とステージを一目で確認

### Task
- Entryを選択してTaskを作成
- 未完了/完了を切り替える
- Entry詳細からもTaskを追加/処理できる

### 保存箱（未整理Clip一覧）
- entry_id 未設定の Clip 一覧（保存日時順）
- Entry化UI（会社候補を確認して作成）
- clip削除

### Chrome拡張ポップアップ（任意入力補助）
- 企業名候補（選択 / 手入力）
- source表示（自動推定）
- 保存ボタン（保存箱行き）
...
| リスク | 対策 |
|--------|------|
| DOM変更で拡張の抽出が壊れる | 100%自動にしない。候補提示 + 確定UIを基本設計に。保存箱でフォールバック |
...
| 高 | メール通知 | 締切3日前/24時間前、日次まとめ |
| 中 | ロードマップ | 長期の就活進捗可視化。βコアには含めない |
...
| Clip / Inbox をβコアへ移動 | 保存箱から Entry 化する流れをコア体験に含める | 保存した求人をすぐ Entry 化しない余白が、入力負荷を下げるため |
